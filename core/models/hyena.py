...
class HyenaEncoderLayer(nn.Module):
    def __init__(self, d_model, l_max, dropout=0.1):
        super().__init__()
    ...
    def forward(self, x):
        x = x + self.dropout(self.hyena(self.norm1(x)))  # attention block
        x = x + self.dropout(self.ff(self.norm2(x)))     # feed-forward block
        return x
