...
from core.models.model import MaskedModeling_FixedModel, ModelConfig
...
if __name__ == "__main__":
    ...
    dataset = DNAMaskDataset(jsonl_file, data_usage_rate=data_usage_rate, tokenizer_name=config.tokenizer_name)
    ...
    best_of_each, best_overall = find_best_architecture(experiment_name, training_strategy="mask", epoch=1)
    ...
    model = MaskedModeling_FixedModel(experiment_name=experiment_name, model_index=best_overall, tokenizer_name=tokenizer_name, embedding_dim=config.embedding_dim)
    ...
    model_save_path = f'/projects/slmreasoning/yifang/re-pretrained/mask/{experiment_name}/'
    ...
    csv_file_path = f"/projects/slmreasoning/yifang/results/{experiment_name}/re-mask-ft"
    ...
    for epoch_num in [0,4]:
        repre_model_path = model_save_path + f'RePretrain-epoch={epoch}.ckpt'
        repre_model = MaskedModeling_FixedModel(experiment_name=experiment_name, model_index=best_overall, tokenizer_name=tokenizer_name, embedding_dim=config.embedding_dim)
        repre_model.load_state_dict(torch.load(model_path))
