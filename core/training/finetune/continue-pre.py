...
if __name__ == "__main__":
    ...
    if strategy == 'contrastive':
        ...
        csv_file_path = output_dir + "con-con_results.csv"

    elif strategy == 'mask':
        CNN_Pretrained_ModelSpace = MaskedModeling_ModelSpace(config)
        continual_pretrain = continual_mask_pretrain
        csv_file_path = output_dir + "mask-mask_results.csv"
    ...
    for epoch_num in tqdm(ckpt_epoch_num_list, desc="Checkpoint Steps"):
        ...
        for task_index in tqdm(range(12), desc=f"Tasks for epoch={epoch_num}", leave=False):
            ...
            for tuple_model in tqdm(models, desc=f"Models for task {task_index}", leave=False):
                ...
                model = tuple_model[1]
                model = continual_pretrain(model, task_index, tokenizer_name=tokenizer_name, data_usage_rate=data_usage_rate)
                acc = cls_evaluate_model(freeze_flag=True, model=model, task_index=task_index, tokenizer_name=tokenizer_name)
                acc = round(acc, 2)
